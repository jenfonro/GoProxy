...
func proxyStream(
	...
	if !headOnly && rangeHeader != "" {
		if start, end, ok := parseRangeBytes(rangeHeader); ok && start >= 0 {
			...
			// Open-ended range usually becomes the primary playback stream. Keep chunk hedge
			// disabled here to avoid request explosion (pool * hedge) on the first window.
			if strings.EqualFold(rangeHeader, "bytes=0-") && effectiveHedgeConns > 1 {
				effectiveHedgeConns = 1
			}
...
func proxyStreamChunked(
	...
	for blockStart := curStart; blockStart <= end; {
		...
		if err := streamChunkBlock(ctx, client, w, target, headers, ifRange, blockStart, blockEnd, chunkSize, poolSize, timeout, retries, chunkHedgeConns, chunkHedgeDelay); err != nil {
			return "", err
		}
...
func streamChunkBlock(
	...
	if poolSize <= 1 {
		for _, j := range jobs {
			...
			body, err := fetchChunkBodyWithHedge(ctx, client, target, headers, rh, ifRange, timeout, retries, chunkHedgeConns, chunkHedgeDelay)
			if err != nil {
				return err
			}
			if len(body) > 0 {
